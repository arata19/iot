<?php	
	...
	function MD7($str)
	{
		return MD5("CCTV" . $str . "BADILAG");
	}
	
	function MD7S($str)
	{
		return "MD5(CONCAT('CCTV',CONCAT($str),'BADILAG'))";
	}
	...
	function cdn_foto($foto1=null, $foto2=null, $size='120'){
		...
		if(!empty($foto1))
		$img = "https://sikep.mahkamahagung.go.id/uploads/foto_formal/" . trim($foto1);
		if(!empty($foto2))
		$img = "https://sikep.mahkamahagung.go.id/uploads/foto_pegawai/" . trim($foto2);
	...
	function get_senin($month,$year){
        ...
        $dates = [];
			for($i = strtotime('Monday', $start); $i <= $end; $i = strtotime('+1 week', $i)){
				$dates[] = date('Y-m-d', $i);
			}
            
        return $dates;        
    }
    function get_jumat($month,$year){
        // Start of month
        $start = mktime(0, 0, 0, $month, 1, $year);
        // End of month
        $end = mktime(0, 0, 0, $month, date('t', $start), $year);
        ...
        $dates = [];
			for($i = strtotime('Friday', $start); $i <= $end; $i = strtotime('+1 week', $i)){
				$dates[] = date('Y-m-d', $i);
			}
            
        return $dates;        
    }
    if ( ! function_exists('tgl_mingguan'))
	{
		function tgl_mingguan($tgl)
		{
    ...
    if ( ! function_exists('cek_tgl_apel'))
	{
		function cek_tgl_apel($tgl)
		{
